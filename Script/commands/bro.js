...
module.exports.run = async function ({ event, api, args }) {    
  ...
  if (!mention[0]) return api.sendMessage("আরে বলদ একজনকে মেনশন তো করবি নাকি 😑🌚", threadID, messageID);
      ...
      return makeImage({ one, two }).then(path => api.sendMessage({ body: "নে তোদের জোড়া দিয়ে দিলাম🙂👍", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
  }
    }
